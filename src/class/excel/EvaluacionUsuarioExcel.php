...
if ($codusuario <> "") { $where.=" AND colaborador='".$codusuario."'"; }
if ($fechaform<> "" and $fechaform != "undefined") { $where.=" AND fecha = '".$fechaform."'"; }

$detalle="Evaluación";

$sql="SELECT * FROM feedback WHERE  ".$where." GROUP BY fecha, colaborador, codformulario ORDER BY fecha ASC";
...
while ($rescont < mysqli_num_rows($res_res)) {
		$fecha=mysqli_result($res_res, $rescont, "fecha");
				 ...
				 $sel_resultado="SELECT * FROM feedback WHERE borrado=0 AND  colaborador='".$codusuario."' AND fecha = '".$fecha."'";
...
//$objPHPExcel->getActiveSheet()->setCellValue('G1', ": ".$sel_resultado);
	...
	$objPHPExcel->getActiveSheet()->setTitle(date('d-m-Y', strtotime($fecha))." - ".trim(mysqli_result($res_familias, 0, "descripcion")));
	} else {
		...
		$z=0;
			...
			while ($contador < mysqli_num_rows($res_resultado)) { 
				...
				if(mysqli_result($res_resultado, $contador, "nivel")!='') {
					$sql_feedback="SELECT competencias, definicion FROM feedbackform WHERE codformulario=".mysqli_result($res_resultado, $contador, "codformulario")." AND fila=".(mysqli_result($res_resultado, $contador, "fila"));
