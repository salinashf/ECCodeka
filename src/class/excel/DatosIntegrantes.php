...
$objPHPExcel->getActiveSheet()->getDefaultStyle("C1:H1")->applyFromArray($tiponormal);
...
/*               */
$objPHPExcel->getActiveSheet()->getDefaultStyle("B4:H4")->getFont()->setName('Arial');
$objPHPExcel->getActiveSheet()->getDefaultStyle("B4:H4")->getFont()->setSize(12);
...
$objPHPExcel->getActiveSheet()->setCellValue('C'.$x, " Cargo ");
...
$ssql="SELECT CONTACTOSMAILPRI,CONTACTOSCELULAR,CONTACTOSTELEFONO,CONTACTOSCUMPLE2,CONTACTOSCUMPLE,CALIDAD.CALIDADDESCRIPCION,ANIOID,ACTIVIDADID,CONTACTOSNOMBRE,CONTACTOSAPELLIDO, 
CONTACTOS.CONTACTOSID as contacto FROM `ORGANIZACION` INNER JOIN CONTACTOS ON CONTACTOS.CONTACTOSID = ORGANIZACION.CONTACTOSID LEFT JOIN CALIDAD ON ORGANIZACION.CALIDADID=CALIDAD.CALIDADID
 WHERE `ANIOID` = '$anio' and `ACTIVIDADID` = '$actividad' ORDER BY CALIDAD.CALIDADID ASC , contacto ASC";
/*
...
if ($DATA['CONTACTOSCUMPLE']!="0000-00-00" AND isset($DATA['CONTACTOSCUMPLE']) ) {
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$x, " ".$CALIDADDESCRIPCION);
...
for ($xx = $x; $xx <= $resto; $xx++) { 
		...
		$objPHPExcel->getActiveSheet()->getStyle('A'.$xx.':H'.$xx)->applyFromArray($border_fino);
