...
//data_last_month_day(explota($_POST['fechafin'])); 
...
<style type="text/css">
	...
	* { margin:0;}
	...
	<?php 
		$cont=0;
		$saldoTotal=0;
		$sql_cliente="SELECT codcliente,nombre, apellido, empresa FROM clientes ";
		$res_cliente=mysqli_query($GLOBALS["___mysqli_ston"], $sql_cliente);
		 while ($cont < mysqli_num_rows($res_cliente)) { 
		...
		$codcliente=mysqli_result($res_cliente, $cont, "codcliente");
	$startTime=$sTime;		
	$totalfactura=0;
	$importe=0;
	$saldo=0;
	
/*Si el saldo es 0 no lo muestro*/	
$totalfactura=0;
$importe=0;
$saldo=0;

 			$sel_resultado="SELECT codfactura,fecha,totalfactura,estado,facturas.tipo,facturas.iva,facturas.moneda
				FROM facturas WHERE facturas.borrado=0 AND facturas.tipo=1 AND codcliente= ".$codcliente." ".$where." AND fecha <= '".$endTime."'";
		   ...
		   while ($contador < mysqli_num_rows($res_resultado)) { 
			...
			FROM recibos WHERE borrado=0  AND codcliente= ".$codcliente." ".$where." AND fecha <= '".$endTime."'";
...
$saldo=0;
 			...
 			$sel_resultado="SELECT codfactura,fecha,totalfactura,estado,facturas.tipo,facturas.iva,facturas.moneda
				FROM facturas WHERE facturas.borrado=0 AND facturas.tipo=1 AND codcliente= ".$codcliente." ".$where." AND fecha < '".$startTime."'";
		   ...
		   while ($contador < mysqli_num_rows($res_resultado)) { 
			...
			$sel_resultado="SELECT codrecibo,fecha,importe,moneda
			FROM recibos WHERE borrado=0  AND codcliente= ".$codcliente." ".$where." AND fecha < '".$startTime."'";
...
?>	
	...
	<td style="text-align:left;width:38.66mm; " class="Default"> </td>
	<td style="text-align:left;width:30.76mm; " class="ce1"> </td>
	<td colspan="2" style="text-align:left;width:33.21mm; " class="ce9"><p>Estado Anterior en <?php echo @$monedas[$moneda];?>: </p></td>
...
?>	
			...
			<td style="text-align:right; width:25.05mm; " class="ce7"><p><?php $saldo+=mysqli_result($res_resultado, $contador, "totalfactura");
...
?>
	...
	<td style="text-align:left;width:24.77mm; " class="ce7"><p>&nbsp;<?php echo $monedas[mysqli_result($res_resultado, $contador, "moneda")];?>&nbsp; </p></td>
	<td style="text-align:left;width:25.05mm; " class="ce7"> </td>
	<td style="text-align:right; width:25.05mm; " class="ce7"><p>
	<?php $saldo-=mysqli_result($res_resultado, $contador, "importe");
...
?>
	...
	<td colspan="2" style="text-align:left;width:25.05mm; " class="ce12"><p>Saldo Final <?php echo @$monedas[$moneda];?>&nbsp;</p></td>
	<td style="text-align:right; width:25.05mm; " class="ce13"><p>
	<?php
		$saldoTotal=$saldoTotal+$saldo;
	 echo number_format($saldo,2,",",".");?>&nbsp;</p></td>
	</tr>
<?php 

	} //Finalizo la condisción de que el saldo sea > 0
$cont++;
}
?>	
	<tr class="ro5">
	...
	<td style="text-align:left;width:33.21mm; " class="ce5"> </td>
	<td style="text-align:left;width:24.77mm; " class="ce5"> </td>
	<td colspan="2" style="text-align:left;width:25.05mm; " class="ce12"><p>Saldo TOTAL Final <?php echo @$monedas[$moneda];?>&nbsp;</p></td>
	<td style="text-align:right; width:25.05mm; " class="ce13"><p>
	<?php echo number_format($saldoTotal,2,",",".");?>&nbsp;</p></td>
	</tr>
	</table>
	</body></html>