...
<?php
/*Recorro todas las evaluaciones agrupadas por tipo de formulario*/
$sql="SELECT feedback.codformulario,feedback.fecha FROM feedback INNER JOIN formularios on formularios.codformulario=feedback.codformulario 
WHERE ".$where." and formularios.borrado=0  and formularios.tipo=0 GROUP BY  feedback.codformulario ORDER BY feedback.fecha ASC";
...
<?php
/*Para cada tipo de formulario recorro cada fecha y tomo el valor de cada evaluación*/
		$sel_resultado="SELECT formularios.descripcion, feedback.fila, feedback.codformulario,feedback.fecha FROM feedback 
		INNER JOIN formularios on formularios.codformulario=feedback.codformulario
		WHERE feedback.borrado=0 AND formularios.tipo=0 AND  feedback.nivel!='' AND colaborador='".$codusuario."' AND fecha = '".$fecha."'";
...
*/
		...
		$evaluacion=mysqli_result($res_resultado, $contador, "descripcion")." - ".mysqli_result($res_familias, 0, "competencias");		
?>
<div class="clear">
<?php  if ($rowControl % 2) { ?>
