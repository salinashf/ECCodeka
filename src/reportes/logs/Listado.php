	...
	foreach ($alldata as $key => $value)
	{
        ...
        $data[$$key]=$temp;
            if(strlen(trim($value)) >0 and $key=='fechaini'){ 
                ...
                $obj->Where('fecha', explota($value)." 00:00:00", '<=');    
            } elseif(strlen(trim($value)) >0 and $key=='oidcontacto') {          
                if(strlen(trim($value)) >0){ 
                    $obj->Where('oidcontacto', $value, '=');    
                }   
            } else{          
                if(strlen(trim($value)) >0){ 
        ...
        buttons: [
            {
                extend: 'print',
                text: 'Imprimir', 
                title:'<strong>Registro de actividad de usuario</strong>',
                autoprint:false,
                footer:true,
