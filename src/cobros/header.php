...
$("#msganio",top.document).html("<i class='fa fa-long-arrow-right' aria-hidden='true'></i>&nbsp; <?php echo _('Cobros/Recibos');?>");
...
$("#modal-btn-psi").on("click", function(){
  callback(true);
  $("#print-modal").modal('hide');
});

$("#modal-btn-pno").on("click", function(){
  callback(false);
  $("#print-modal").modal('hide');
}); 

$("#modal-btn-msi").on("click", function(){
  callback(true);
  $("#mail-modal").modal('hide');
});

$("#modal-btn-mno").on("click", function(){
  callback(false);
  $("#mail-modal").modal('hide');
}); 
};
...
<script type="text/javascript">
function eliminar(cod, codtabla, estado){
  if(estado==1){
    showToast('El recibo ya fue emitido','error'); 
  }else{
    $("#Sendcod").val(cod);
    $("#mi-modal").modal('show');
    modalConfirm(function(confirm){
        ...
        $("#mi-modal").modal('hide');
            if (password == "1234" && password.length>0) {
              $('#inputPassword').val('');
              var cod=$("#Sendcod").val();
              var datasend={"cod":cod, "tabla":codtabla};
...
function modificar(codrecibo) {
  $("#mailModalLabel").html("Confirmar modificar documento?");
  $("#PassSubmitMail").show();
    //console.log('enviar '+codrecibo);
  $("#mail-modal").modal('show');
    modalConfirm(function(confirm){
      if(confirm){
      $('#loginModal').modal('show');
      $("#PassSubmitMail").on("click", function(){
      ...
      $("#mail-modal").modal('hide');
          if (password == "1234" && password.length>0) {
            ...
            var cod=$("#Sendcod").val();
            if(cod!=''){
              OpenWindow('edit.php?codrecibo=" . $codrecibo ."', 'frame_rejilla','98%','98%',true, true,true, 'form');  
            }else{
              parent.showToast('<?php echo _('Se produjo un error, intente más tarde');?>', 'error');
            }
          } else {
            if(showone==0 && password.length>0){
              showone=1;
              parent.showToast('<?php echo _('Contraseña erronea');?>', 'error');
            }            
          }
          $('#inputPassword').val('');
          $('#loginModal').modal('hide');        
...
<script languge="javascript">
  ...
  function validar() 	{
    ...
    var mensaje='';
    var Index=$('#Atipo').find('option:selected').val();
    var codentidad=$('#codentidad').find('option:selected').val();
    var Moneda=$('#moneda').find('option:selected').val();

    event.preventDefault();

    if(Index==0){
      mensaje+=' - Falta seleccionar forma de pago.<br>';
      $('#Atipo').focus();
      
    }
    if(Moneda==-1){
      mensaje+=' - Falta seleccionar moneda.<br>';
      $('#Atipo').focus();
      
    }
    if(codentidad==-1 && Index!=1){
      mensaje+=' - Falta seleccionar Banco.<br>';
      $('#Atipo').focus();
      
    }
    if(Index!=1 && Index!=3){
      if ($("#numeroserie").val()=="") mensaje+=" - Falta nº serie<br>";
      if ($("#numero").val()=="") mensaje+=" - Falta nº doc.<br>";
    }

    if ($("#importe").val()=="" || $("#importe").val()==0 ) { 
      mensaje+=" - Falta importe<br>"; 
    } else {
      if (isNaN($("#importe").val())==true) {
        mensaje+=" - El importe debe ser numerico<br>";
      }
    }
  ...
  function enviar_recibo(codrecibo, enviar) {
  
  $("#mailModalLabel").html("Confirmar reenviar documento a cliente?");
  $("#Sendcod").val(codrecibo);
  if (enviar != 1) {
    parent.showModal(1, 'Enviando documento por mail');
    var cod=$("#Sendcod").val();
    if(cod!=''){
      var datasend={"codrecibo":cod, "envio":'1'};
      $.ajax({ 
        type: "POST",
        url: "../fpdf/imprimir_recibo_mail.php",
        cache: false,
        data: datasend,
        success: function(text){
          var jsontext = jQuery.parseJSON(text);                    
          parent.showModal(0);
          $("#Sendcod").val('');
          parent.showToast(jsontext[0].status, 'info');
          $("#form").submit();
        },
  ...
  function agregar_factura(codfactura, totalfactura) {

    $("#acodfactura").val(codfactura);
    $("#totalfactura").val(totalfactura);
    document.formulario_facturas_sel.submit();

  }
  function pon_total(total){
    $('#totalfacturas').val(parseFloat(total));
    calcularsaldo();
      $('#frame_lineas').attr('src',$('#frame_lineas').attr('src'));
      //$('#frame_lineas').attr( 'src', function ( i, val ) { return val; });

  }
  function pon_cobro(cobro){
    //console.log(cobro);

    $('#totalrecibo').val(parseFloat(cobro));
    calcularsaldo();
  }
  function calcularsaldo(){
    var total = $('#totalfacturas').val();
    var cobro = $('#totalrecibo').val();
    var saldo = total - cobro;
    $('#saldo').val(saldo);
  }
  ...
  function Guardodatos() {
    ...
    var mensaje="";
    if ($("#codcliente").val()=="") mensaje+="  - Nombre<br>";
    if ($("#fecha").val()=="") mensaje+="  - Fecha<br>";
    if ($("#codrecibo").val()=="") mensaje+="  - Cod. recibo<br>";
    if ($("#Amoneda").val()=="") mensaje+="  - Seleccione moneda<br>";			
    if (mensaje!="") {
      showToast("Errores detectados:<br>"+mensaje, 'error');
      return false;
    }
    ...
    var fecha = $('#fecha').val();
    var Amoneda = $('#Amoneda').val();
    var tipocambio = $('#tipocambio').val();
    var totalfacturas = $('#totalfacturas').val();
    var totalrecibo = $('#totalrecibo').val();
    var saldo = $('#saldo').val();
    ...
    $.ajax({ 
        ...
        url: "guardodatos.php",
        cache: false,
        data: {"codrecibo": ""+codrecibo+"", "fecha": ""+fecha+"", "Amoneda": ""+Amoneda+"",
          "totalfacturas": ""+totalfacturas+"", "totalrecibo": ""+totalrecibo+"", "codcliente": ""+codcliente+""},
        dataType: "json",
        success: function(data){
          var json = JSON.stringify(data);
          var json_obj = $.parseJSON(json);
          var mensaje =json_obj[0].msg;
          
          parent.showToast(mensaje, 'info');
