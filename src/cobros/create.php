...
$mensaje='';
    ...
    $codfactura=isset($_GET['codfactura']) ? $_GET['codfactura'] : '';
    ...
    clientes.apellido, clientes.empresa, facturas.totalfactura,facturas.estado, facturas.moneda ');
    $obj->Join('codcliente', 'clientes', 'INNER', 'facturas', 'codcliente' );

    $obj->Where('codfactura', $codfactura, '', '', '', 'facturas');
    ...
    $preciototal=0;
    $codrecibotmp=strtotime(date('Y-m-d H:m:t'));

    $objRecibo = new Consultas('recibos');
    $objRecibo->Select();
    $objRecibo->Orden('codrecibo', 'DESC');
    $objRecibo->Limit(0,1);
    $resRecibo=$objRecibo->Ejecutar();

    $rowsRecibos=$resRecibo['datos'][0];

   $codrecibo = (int)$rowsRecibos['codrecibo']+ 1;

    $objRecibo = new Consultas('recibosfacturatmp');
    $objRecibo->Select();
    $objRecibo->Orden('codrecibo', 'DESC');
    $objRecibo->Limit(0,1);
    $resRecibo=$objRecibo->Ejecutar();

    if($resRecibo['numfilas']>0){
        $proceso=$resRecibo['datos'][0];
        if($codrecibo<=$proceso['codrecibo']){
           $codrecibo++;
        }
    }
...
<div class="row">
    ...
    <?php if(!$_POST)
        ...
        <?php
$arr = [ 'codrecibo' => $codrecibo ];
?>
        event.preventDefault();
        cancelar('recibosfacturatmp', <?=htmlentities(json_encode($arr))?>);  " >
        <span class='glyphicon glyphicon-ban-circle' data-dismiss="modal"></span> <?php echo _('Cancelar'); ?></button>
    </div>
    </div>
</div>
</form>
