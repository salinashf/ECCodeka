...
function eliminar_linea(cod, codrecibo, tabla){
    var datasend={"cod":cod, "codrecibo":codrecibo, "tabla":tabla};
    var showone=0;
    $.ajax({ 
    type: "POST",
    ...
    success: function(text){
            ...
            if(n>0){
                tipo="error";
                text='No se pudo quitar la línea';
            }else{
                tipo="success";
                text='Línea quitada';
                console.log('tabla '+tabla);
                if(tabla=='recibospagotmp'){
                    //parent.$('#frame_pago').attr( 'src', function ( i, val ) { return val; });
                    //parent.rejilla_pago.href='rejilla_pago.php?codrecibo='+codrecibo;
                    parent.document.rejilla_pago.submit();
                    //$('#iframeid',window.parent.document).attr('src',$('#iframeid',window.parent.document).attr('src'));
                }
                if(tabla=='recibosfacturatmp'){
                    parent.$('#acodfactura').val('');
                    var codcliente = parent.$('#codcliente').val();
                   
                    window.parent.frame_lineas.href='rejilla_facturas.php?codcliente='+codcliente;
                   
                    $('#frame_lineas',window.parent.document).attr('src',$('#frame_lineas',window.parent.document).attr('src'));
                    $('#frame_lineas', window.parent.document).attr( 'src', function ( i, val ) { return val; });
                   
                    
                    //window.parent.formulario_facturas_sel.href='rejilla_facturas_sel.php?codrecibo='+codrecibo;
                    //window.parent.document.getElementById("frame_facturas_sel").contentDocument.location.reload(true);
                    
                    //$('#frame_facturas_sel',window.parent.document).attr('src',$('#frame_facturas_sel',window.parent.document).attr('src'));
                    //$('#formulario_facturas_sel', window.parent.document).attr( 'src', function ( i, val ) { return val; });
                    parent.document.formulario_facturas_sel.submit();
                    //parent.location.href=parent.location.href;
                    //window.parent.document.getElementById("frame_facturas_sel").contentDocument.location.reload(true);
                   
                    console.log('recibosfacturatmp');
...
function agregar_factura(codfactura, totalfactura){
    parent.agregar_factura(codfactura, totalfactura);
}
function enviar_recibo(codrecibo, envio) {
    parent.enviar_recibo(codrecibo, envio);
}
function eliminar(codrecibo, tabla, estado) {
    parent.eliminar(codrecibo, tabla, estado);
}
</script>
