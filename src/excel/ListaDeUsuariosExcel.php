...
$Atratamiento=isset($_GET['Atratamiento']) ? @$_GET['Atratamiento'] : null ;
$estado=isset($_GET['estado']) ? @$_GET['estado'] : null ;
$telefono=isset($_GET['telefono']) ? @$_GET['telefono'] : null ;
$nempleado=isset($_GET['nempleado']) ? @$_GET['nempleado'] : null ;

$where=" AND 1=1";
if ($nombre <> "") { $where.=" AND (nombre like '%".$nombre."%' or apellido like '%".$nombre."%') "; }
if ($apellido <> "") { $where.=" AND (nombre like '%".$apellido."%' or apellido like '%".$apellido."%') "; }
if ($estado <> "") { $where.=" AND estado='".$estado."' "; }
if ($telefono <> "") { $where.=" AND telefono like'%".$telefono."%' "; }
if ($nempleado <> "") { $where.=" AND nempleado ='".$nempleado."' "; }
if ($Atratamiento <> "") { $where.=" AND tratamiento ='".$Atratamiento."' "; }
...
//comentar sifuiente línea en producción
//$objPHPExcel->getActiveSheet()->setCellValue('J'.$x, $sel_resultado);
