...
//codproveedor+"+nif+"&provincia="+provincia+"&localidad="+localidad+"&telefono="+telefono;
$codproveedor=isset($_GET['codproveedor']) ? $_GET['codproveedor'] : null ;
...
if ($provincia > 0) { $where.=" AND provincia = '".$provincia."'"; }
if ($nif > "0") { $where.=" AND nif='$nif'"; }
if ($localidad <> "") { $where.=" AND localidad like '%".$localidad."%'"; }
if ($telefono <> "") { $where.=" AND telefono like '%".$telefono."%'"; }

$where.=" ORDER BY codproveedor ASC";
$detalle="Listado de proveedores";

$objPHPExcel->getActiveSheet();
$objPHPExcel->getActiveSheet()->setTitle('Listado de proveedores ');
$celda="H";
...
$objPHPExcel->getActiveSheet()->setCellValue('B'.$x, "Proveedor");
$objPHPExcel->getActiveSheet()->setCellValue('C'.$x, "RUT");
...
$objPHPExcel->getActiveSheet()->setCellValue('H'.$x, "Movil");
...
$x++;
$sel_resultado="SELECT * FROM proveedores WHERE `borrado`=0 ";//.$where;
...
// Add some data
$objPHPExcel->getActiveSheet()->setCellValue('A'.$x, mysqli_result($res_resultado, $contador, "codproveedor")." ");
...
$objPHPExcel->getActiveSheet()->setCellValue('H'.$x,  " ".mysqli_result($res_resultado, $contador, "movil"));
$objPHPExcel->getActiveSheet()->getStyle('H'.$x)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
