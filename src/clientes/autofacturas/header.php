...
$("#msganio",top.document).html("<i class='fa fa-long-arrow-right' aria-hidden='true'></i>&nbsp; <?php echo _('Listado autofacturas');?>");
...
<script type="text/javascript">
function eliminar(cod, codcliente, codtabla){
  $("#Sendcod").val(cod);
...
$(document).keydown(function(e) {
    switch(e.keyCode) { 
        ...
        var index = Number($this.attr('data-index'));
	        ...
	        if (index==2) {
	        	var cfactura=$("#Acodautofactura").val();
	        		if (cfactura=='') {
...
<script languge="javascript">
  ...
  function validar() 	{
				...
				var entero=0;
				var enteroo=0;
        var codautofactura=$('#Acodautofactura').val();
        var Index=$('#Amoneda').find('option:selected').val();
        var Iva=$('#Aiva').find('option:selected').val();
        ...
        if(Iva==-1){
          showToast('Falta seleccionar IVA', 'error');
          $('#Aiva').focus();
          return false;
        }
		...
		function validar_cabecera()
			{
				...
				if ($("#Acodautofactura").val()=="") mensaje+="  - Cod. Factura<br>";
...
function compruebonumerofactura(){
  ...
  var codautofactura = $('#Acodautofactura').val();
  var tabla="autofacturas";
  
    var datasend={"cod":codcliente, "codautofactura":codautofactura, "tabla":tabla};
    $.ajax({ 
      type: "POST",
