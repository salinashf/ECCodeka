...
if($cod != 'undefined' and strlen($tabla) >0){
	...
	if($tabla=='autofacturas'){
		$nom[] = 'borrado';
		$val[] = 1;
		$obj->Update($nom, $val);
		$obj->Where('codautofactura', $cod, 'LIKE');
		$hace = _('Borra autofactura'). $cod;
	}	
	$algo = $obj->Ejecutar();

	if ( $algo['estado']=='ok' ){ 
		echo _('Borrado con exito').$algo['consulta'];
		$hace = _('Da de baja autofactura: '). $cod;
		logger($UserID, $oidestudio, $oidpaciente, $hace);
	}else{
		$hace = _('Fallo dar de baja autofactura: '). $cod;
		logger($UserID, $oidestudio, $oidpaciente, $hace);
