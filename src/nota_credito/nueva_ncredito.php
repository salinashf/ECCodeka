...
$codncredito=isset($_GET['num']) ? $_GET['num'] : null;

$sel_fact="SELECT * FROM `ncredito` WHERE `codncredito` ='$codncredito'";
...
if ( mysqli_num_rows($rs_fact)>0) { 
	if(mysqli_result($rs_fact, 0, "estado")==1 and mysqli_result($rs_fact, 0, "tipo")==0){
		echo "<script language=\"javascript\">
				showWarningToast(\"No puede modificar una nota de crédito ya emitida.\");
				parent.$('idOfDomElement').colorbox.close();</script>";

	} else {
		header('Location: modificar_ncredito.php?codfactura='.$codncredito);
	}
}
...
if(@$_GET['fecha']=='') {
	...
	$sel_fact="SELECT codncredito FROM `ncredito` ORDER BY codncredito DESC, fecha DESC LIMIT 1 ";
	$rs_fact=mysqli_query($GLOBALS["___mysqli_ston"], $sel_fact);
	if(mysqli_num_rows($rs_fact)>=1) {
	$codncreditotmp=$codncredito=(int)mysqli_result($rs_fact, 0, "codncredito")+1;
	} else {
	$codncreditotmp=$codncredito=1;
	}
} else {
...
var Index = document.formulario.Amoneda.options[document.formulario.Amoneda.selectedIndex].value;
	if (Index!=0) {
		...
		showWarningToast("Seleccione Moneda:<br>");	
	}
				...
				<form id="formulario" name="formulario" method="post" action="guardar_ncredito.php">
					<table class="fuente8" cellspacing=0 cellpadding=2 border=0>
						<tr>
					      ...
					      <td><input name="codcliente" type="text" class="cajaPequena" id="aCliente" size="6" maxlength="5" value="" data-index="1">
							...
							<td>Nombre</td>
						    <td colspan="5"><input name="nombre" type="text" class="cajaGrande" id="nombre" size="45" maxlength="45"  readonly></td>
						    <td>RUT</td>
						  ...
						  <td colspan="2"><input name="codncredito" class="cajaPequena" readonly="" value="<?php echo $codncredito;?>" readonly ></input></td>
							<td>Moneda</td><td> <select onchange="cambio();" name="Amoneda" id="Amoneda" class="cajaPequena2" data-index="2">
						...
						<tr>
							<td>Fecha</td><td>
							<input name="fecha" type="text" class="cajaPequena" id="fecha" size="10" maxlength="10" value="<?php echo implota($fechahoy);?>"  data-index="3" readonly> 
				      ...
				      <td><input name="iva" type="text" class="cajaPequena" id="iva" size="5" maxlength="5" onChange="cambio_iva()" value="<?php echo $iva;?>"> %</td>
				       <td colspan="4">Tipo&nbsp;Cambio
						...
						<td colspan="3"><label>Agencia&nbsp;</label><span><input name="agencia" type="text" class="cajaGrande" id="agencia" size="5" maxlength="5" value=""></span>
						  &nbsp;</td>				         					        					
			  ...
			  <!-- Sección listado de facturas del cliente -->
				...
				<table class="fuente8" cellspacing=0 cellpadding=2 border=0>
				  <tr>
					...
					<td colspan="2" valign="top">Comisión&nbsp;<input name="comision" type="text" class="cajaMinima" id="comision" size="10" maxlength="10" readonly style="background-color: yellow;" >&nbsp;%</td>
					 
				  </tr>
				  <tr>
  				  <?php if($shoedetalle==1) { ?>
					<td valign="top">Detalles</td>
					<td colspan="2"><textarea name="detalles" rows="1" cols="100" class="areaTexto" id="detalles" readonly> </textarea></td>
					<?php } ?>
