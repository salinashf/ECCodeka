...
<head>
	...
	<link href="estilo_archivo.css" rel="stylesheet" type="text/css">
<style>
#myProgress {
    position: relative;
    width: 100%;
...
<tr><td>Equipo/Lugar</td>
<td>
<input type="hidden" name="codbiometric" value="<?php echo $codbiometric;?>" />
<?php
...
if($Eje_rows>0){
  ...
  foreach($urows as $urow){
	$lugar=$urow['lugar'];
	$ubiobj = new Consultas('ubicaciones');
	$ubiobj->Select();
	$ubiobj->Where('codubicacion', $urow['codubicacion'], '=');

	$ubiEje = $ubiobj->Ejecutar();
	$ubiEje_rows=$ubiEje["numfilas"];
	if($ubiEje_rows>0){
	  $ubirows = $ubiEje["datos"]; 
	  foreach($ubirows as $ubirow){
		$nombre=$ubirow["nombre"];
		if(strlen($lugar)>0) {
			$nombre.=" / ".$lugar;
		}
...
<input id="equipo" name="equipo" class="comboGrande" value="<?php echo $nombre;?>"></td>
...
$(my_form_id).on( "submit", function(event) { 
	...
	if ($("#equipo").find('option:selected').val()==0) {
