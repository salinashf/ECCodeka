...
$("#msganio",top.document).html("<i class='fa fa-long-arrow-right' aria-hidden='true'></i>&nbsp; <?php echo _('Ventas');?>");
...
<script type="text/javascript">
function eliminar(cod, codcliente, codtabla, emitida){
  if(emitida==0){

  $("#myModalLabel").html("Confirmar eliminar documento?");
  $("#Sendcod").val(cod);

  $("#PassSubmitDel").show();

  $("#mi-modal").modal('show');
  //console.log('tabla '+codtabla);
    modalConfirm(function(confirm){
      if(confirm){

        $('#loginModal').modal('show');
        $("#PassSubmitDel").on("click", function(){
        ...
        $("#PassSubmitDel").hide();
          if (password == "1234" && password.length>0) {
            $('#inputPassword').val('');
            var cod=$("#Sendcod").val();
            var datasend={"cod":cod, "codcliente": codcliente, "tabla":codtabla};
    ...
    $("#loginModal").on("hide.bs.modal", function () {
     ...
     showToast('Factura ya emitida, no se puede borrar','error'); 
  }
}
        ...
        var index = Number($this.attr('data-index'));
	        if (index==1) {
	        	var codigo=$("#Acodcliente").val();
	        		if (codigo=='') {
                showToast('<?php echo _('Debe introducir antes el cliente');?>', 'error');
	        			$('[data-index="1"]').focus();
	        			return false;
		        	}
		     }
...
<script languge="javascript">
  ...
  function validar() 	{
        ...
        var Acodcliente=$('#codcliente').val();

				event.preventDefault();

        if(Acodcliente==""){
          showToast('Falta seleccionar cliente', 'error');
          $('#Acodcliente').focus();
          return false;
        }
        if(codfactura==""){
...
function compruebonumerofactura(){
  ...
  var tabla="facturasp";
    ...
    var datasend={"cod":codcliente, "codfactura":codfactura, "tabla":tabla};
    $.ajax({ 
      type: "POST",
...
function imprimir(codfactura, emitida) {
  ...
  $("#PrintModalLabel").html("Confirmar reimprimir documento?");
  $("#Sendcod").val(codfactura);
  if (emitida != 1) {
    var cod=$("#Sendcod").val();
    if(cod!=''){
      var datasend={"codfactura":cod, "envio":'0'};
      $.ajax({ 
        type: "POST",
        url: "../fpdf/imprimir_factura_mail.php",
        cache: false,
        data: datasend,
        success: function(text){
          //console.log(text);
              var printer = text.printer; 
              var print_id = text.print_id;
              parent.CallexeDiag(1,print_id,cod, printer); 
              $("#Sendcod").val('');
    ...
    $("#PassSubmitPrint").show();
    //console.log('imprimir '+codfactura);
  $("#print-modal").modal('show');
    modalConfirm(function(confirm){
        if(confirm){
        $('#loginModal').modal('show');
        $("#PassSubmitPrint").on("click", function(){
          $("#PassSubmitPrint").hide();
          var password = $('#inputPassword').val();
          var showone=0;
          $("#print-modal").modal('hide');
          if (password == "1234" && password.length>0) {
...
function enviar_factura(codfactura, enviar) {
  ...
  if (enviar != 1) {
    ...
    var cod=$("#Sendcod").val();
    if(cod!=''){
      var datasend={"codfactura":cod, "envio":'1'};
